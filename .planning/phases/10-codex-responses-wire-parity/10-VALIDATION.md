...
| Delta-only completed response is non-empty; terminal text is not duplicated; genuine empty retry remains bounded | OPS-04 | T-10-04, T-10-05 | sampler stream unit test plus `test_actor` request counter |
| Cross-provider switch retains ordinary context but removes foreign encrypted payloads | OPS-05 | T-10-06, T-10-07 | model-switch unit/integration capture tests |
| Trusted headers do not leak and session metadata remains stable | OPS-04 | T-10-08, T-10-09 | `codex_reconstruct` trusted/BYOK/custom/xAI matrix |
| Encrypted-content variants are classified only as HTTP 400 recovery cases | OPS-05 | T-10-10 | sampling-types error predicate and shell bounded-recovery tests |
...
- [ ] Each plan must name its concrete regression tests and focused `cargo test` command.
