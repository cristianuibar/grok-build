...
|----|----------|----------------------------|
| D-01 | Omit `Task.model` → inherit parent | tools omit-effort/model inherit + Plan 04 effective-model preflight |
...
| D-11 | Parent model unchanged | `p7_parent_model` after **real spawn** |
| D-12 | Dual fake-token Authorization + base_url **both dirs** | `p7_isolation_grok_parent_codex` + `p7_isolation_codex_parent_grok` **mock HTTP required** (not resolve-only; not one-dir waiver) |
| D-13 | NL via Task schema/docs | schema + agent `p7_` description filters |
| D-14 | AGENT-01 hard gate | resume / effort / roles / personas + `p7_same_provider` (+ lifecycle if present) |
...
Filters below are **planned** names from Plans 01–06 (post review cycle 1). Plan 01 lands
**green** discovery scaffolds; Plans 02–05 green product filters; Plan 06 refreshes Exists?
and greened names from SUMMARYs.
...
| AGENT-03 | Effort threads to overrides (canonical) | 02 | tools · `--lib` · `p7_reasoning_effort` | pass | ❌ Plan 02 |
| AGENT-03 | max alias → xhigh canonical | 02 | tools · `--lib` · `p7_reasoning_effort_max` | pass | ❌ Plan 02 |
| AGENT-03 | Invalid effort reject at Task | 02 | tools · `--lib` · `p7_invalid_reasoning_effort` | pass | ❌ Plan 02 |
| AGENT-03 | Tool invalid / unsupported effort fail-closed in shell | 03 | shell · `--lib` · `p7_invalid_effort` | pass | ❌ Plan 03 |
...
- `p7_isolation_grok_parent_codex_child_route` (+ Authorization **mandatory**)
- `p7_isolation_codex_parent_grok_child_route` (+ Authorization **mandatory**)
- `p7_isolation_never_cross_slot_on_child_seed`
- `p7_isolation_reasoning_effort_medium_on_child_config`
- `p7_parent_model_unchanged_after_cross_provider_spawn` (real spawn)
- `p7_missing_child_codex_no_wrong_backend_request`
- `p7_missing_child_xai_no_wrong_backend_request`
- `p7_same_provider_*` (AGENT-01 anchor / lifecycle)
...
|-----------------------------|--------|
| 1. Same-provider no regression | Plan 01 anchors + Plan 06: `reasoning_effort_explicit`, `resume_model_pinning`, `role_default_used` / `persona_resolved`, `p7_same_provider` / lifecycle |
...
| 3 | 04 | tools `p7_eager` (effective model); shell `p7_credential_gate` injection | 2/2 automated; omit-model + role pin covered |
| 4 | 05 | test seam + `p7_isolation` **both Authorization dirs** + `p7_missing` + `p7_parent_model` real spawn | 2/2 automated; D-12 hard bar both dirs |
| 5 | 06 | **Update** VALIDATION names + PHASE-GATE per-subgroup discover+execute green-only | docs + green filters; AGENT-01 roles/personas + lifecycle if present |

> **Wave alignment:** Plan 01 is **frontmatter `wave: 1`** — green harness / Nyquist scaffolding (historically “Wave 0” in research prose; execution wave number is **1**). Plans 02 and 03 are **wave 2** (parallel; no shared `files_modified` conflict after review — Plan 03 may touch config/auth for pure helper). Plan 04 is **wave 3**. Plan 05 is **wave 4**. Plan 06 is **wave 5**.
...
**Tools (`xai-grok-tools --lib`):**
- `p7_task_tool_input_schema` (or schema filter covering reasoning_effort after Plan 02)
- `p7_reasoning_effort` (wire + invalid + max→xhigh)
- `p7_eager`

**Shell integration / seam:**
- `p7_wave0_harness_smoke` (or retained smoke name)
- `p7_spawn_missing_provider`
- `p7_tool` (unknown model existing-reject)
- `p7_isolation` including **both** direction name stems
- `p7_missing`
- `p7_parent_model`
- `p7_same_provider` and/or `p7_spawn_same_provider`

**Shell lib (`--lib`):**
- `p7_invalid_effort` (Tool provenance fail-closed + fixtures)
- `p7_credential_gate` (Plan 04 effective-model injection)
...
**Agent crate:** `p7_` description/effort guidance filter from Plan 02.

**D-12 checklist (Plan 06):**
- [ ] Test seam documented in Plan 05 SUMMARY
- [ ] `p7_isolation_grok_parent_codex` green with Authorization
- [ ] `p7_isolation_codex_parent_grok` green with Authorization
- [ ] Neither direction is resolve-only only
...
| `nyquist_compliant` | `false` | partial | `true` when all green subgroups pass + D-12 checklist |
...
| `reviews_cycle` | `1` (Codex replan incorporated) | keep | keep |
