...
/// Assert the active `~/.bum/bin/bum` resolves to the expected versioned
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("bum");
    assert!(link.is_symlink(), "bum must be a symlink");
    let resolved = dunce::canonicalize(&link)
        .unwrap_or_else(|e| panic!("active bum symlink does not resolve: {e}"));
    assert_eq!(
        resolved.file_name().unwrap().to_string_lossy(),
        format!("grok-{version}-{platform}"),
        "active bum must be the expected version"
    );
    assert_eq!(
    ...
    let ran_ok = std::process::Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ran_ok, "active bum must pass the smoke-test");
}

/// Lay down a managed-install layout in the test BUM_HOME:
/// `bin/bum -> ../downloads/grok-<version>-<platform>` (what
...
// installers that actually maintain the managed `~/.bum/bin/bum` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.bum/downloads cleanup), the probe must report None — otherwise
