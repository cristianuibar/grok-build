...
mod tests {
    ...
    /// `bundle::bundled_root()` (product-home OnceLock) with multi-scenario
    /// env mutation — inject the root instead (D-WRITERS / OnceLock-safe).
    fn test_bundled_root(tmp: &TempDir) -> PathBuf {
    ...
    fn status_reports_no_cache_when_manifest_missing() {
        ...
        let status = status_bundle_at(&test_bundled_root(&tmp)).unwrap();
        assert_eq!(
            status,
            BundleStatusResult {
                has_cache: false,
    ...
    fn status_reports_cached_entries_from_manifest_and_disk() {
        let tmp = TempDir::new().unwrap();
        let root = test_bundled_root(&tmp);
