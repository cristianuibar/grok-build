...
///
/// For a **managed install** — the running binary lives under product home
/// (e.g. `~/.bum/...`) — prefer the managed `~/.bum/bin/bum` symlink. After an
/// auto-update or `bum update` atomically swaps that symlink, `current_exe()`
...
/// not under product home), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.bum/bin/bum` only when `current_exe()` is unavailable.
...
/// Binary file name for the managed product install (`bum` / `bum.exe`).
fn managed_grok_bin_name() -> &'static str {
    if cfg!(windows) { "bum.exe" } else { "bum" }
}
...
mod tests {
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let target_v2 = bin_dir.join("bum-v2");
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let new_target = bin_dir.join("bum-v2");
        ...
        let stale_target = bin_dir.join("bum-v1");
