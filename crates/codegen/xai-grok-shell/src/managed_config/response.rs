...
pub enum ManagedConfigError {
    ...
    #[error(
        "Your team sign-in was rejected. It may have expired or lack access. Run `bum login` to sign in again."
    )]
    TeamAuthRejected,
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.bum. Make sure the directory exists and is writable.\n  ({0})"
    )]
