...
//! 4. Smoke-validate session JSON under `$BUM_HOME/sessions/` after exit.
//!
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let grok_home = home.join(".bum");
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _grok_guard = EnvVarGuard::set("BUM_HOME", &grok_home);
    ...
    // grok_home() caches the first BUM_HOME per process; a separate test
...
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/BUM_HOME via `cmd.env` below; this global env
    ...
    // Smoke: session storage under BUM_HOME/sessions has JSON artifacts.
