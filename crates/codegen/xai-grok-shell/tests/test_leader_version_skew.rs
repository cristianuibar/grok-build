...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".bum").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
...
async fn new_client_evicts_old_leader_and_sessions_reload() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            std::fs::create_dir_all(home.path().join(".bum")).unwrap();
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let control = LeaderClient::connect(
                home.path().join(".bum").join("leader.sock"),
                "grok-pager-update",
                ClientMode::Stdio,
...
async fn eviction_leaves_single_leader_and_single_auth_owner() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let auth_path = home.path().join(".bum").join("auth.json");
