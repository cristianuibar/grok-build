...
name: create-skill
description: >
  Interactively create a new bum skill (SKILL.md + optional scripts/references).
  Use when the user wants to create a skill, scaffold a skill, or runs /create-skill.
metadata:
  short-description: "Create a new bum skill"
---

# Create Skill

Interactively gather requirements from the user and create a fully working bum skill on disk.
...
2. **Scope** - present the user with two options:
   - **Project** (Recommended): `<repo-root>/.grok/skills/<name>/SKILL.md` - available only in this repo, shareable with teammates (project dir name is still `.grok` for discovery compatibility)
   - **User**: `~/.bum/skills/<name>/SKILL.md` (or `$BUM_HOME/skills/...`) - available in all projects
...
- Trigger phrases and keywords so bum knows when to auto-invoke it
...
2. Tell the user the skill is ready and how to use it:
   ...
   - Automatic: bum will invoke it when the description matches user intent
