...
impl LeaderStdioClient {
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            ...
            .env("GROK_LEADER_SOCKET", home.join(".bum").join("leader.sock"))
            ...
            // ~/.bum/leader.log — keep it chatty for diagnosis.
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".bum").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".bum").join("leader.log")).unwrap_or_default()
}
