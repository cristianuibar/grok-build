...
fn ensure_local_grok_binary(binary: &Path) {
    ...
    let output = Command::new(&cargo)
        ...
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build bum: {e}"));

    assert!(
        output.status.success(),
        "failed to build bum for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
...
/// Resolve product binary: `GROK_BINARY` env (CI), `CARGO_BIN_EXE_bum`, or a
/// locally built `bum` artifact from `xai-grok-pager-bin`.
...
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        // HOME alone does not sandbox product home on Windows: the product
        // resolves `~` via `USERPROFILE`/Known Folders (`std::env::home_dir()`),
        // so without an explicit BUM_HOME every spawned child shares the real
        // `%USERPROFILE%\.bum` — test 1's models_cache.json (which embeds
