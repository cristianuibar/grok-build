...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$BUM_HOME` so harness-injected homes (PTY e2e) always
        // win over a OnceLock that may have been initialised earlier with a
        // different path in the same process graph.
        Self::from_env_home()
    }

    /// Resolve cache paths from the live process environment (not only the
    /// `grok_home()` OnceLock). A seeded `$BUM_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home. Never reads the legacy product-home env key.
    fn from_env_home() -> Self {
        let home = std::env::var_os("BUM_HOME")
    ...
    /// `$BUM_HOME` (or `grok_home()`) so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`BUM_HOME` /
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$BUM_HOME` even when offline mode was not
...
mod tests {
    ...
    /// `$BUM_HOME` env so tests never race the parallel harness.
