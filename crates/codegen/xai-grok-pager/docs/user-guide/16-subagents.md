...
| **How you set them** | At startup, or with agent definitions (`.md` files in project-local `.grok/agents/` or user-global `~/.bum/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under project-local `.grok/personas/`; applied during subagent resolution |
...
Disable subagents with the retained internal compatibility variable or the config file:
...
bum discovers file-based personas from these locations, in priority order:

- `.grok/personas/*.toml` (project)
- `~/.bum/personas/*.toml` (user)
...
> **Note:** bum applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, bum resolves the effective model and reasoning effort in this order, highest priority first:
...
bum manages worktrees through the inherited `x.ai/git/worktree/*` protocol extension methods, including an apply operation that merges changes back into the main working directory.
...
bum also discovers roles from project-local `.grok/roles/*.toml` and `.grok/personas/*.toml` compatibility directories. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

bum shows running and finished work in side panes on the agent screen:
