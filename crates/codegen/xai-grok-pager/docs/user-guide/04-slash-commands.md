...
When the context window fills up, bum auto-compacts at 85% usage (configurable via `[session] auto_compact_threshold_percent` in config.toml).
...
choice persists to `[ui].screen_mode` in `config.toml`, so future plain `bum`
...
Run a prompt on a recurring interval. Specify the interval as `30m`, `1 hour`, or `every 2 days`. If you omit it, bum prompts you.
...
Set, manage, or check an autonomous goal. bum works toward the objective across turns and reports progress.
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter appears as a slash command. (A skill turned off via `/skills` is not advertised.) For example, if you have a skill at `~/.bum/skills/commit/SKILL.md` (or `$BUM_HOME/skills/commit/SKILL.md`), you can invoke it with:
