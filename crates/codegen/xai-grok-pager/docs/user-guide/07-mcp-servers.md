...
MCP (Model Context Protocol) servers extend bum with external tool integrations. They let bum interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to bum over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside bum's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.bum/config.toml` (or `$BUM_HOME/config.toml`) under `[mcp_servers.<name>]` sections.

### stdio Transport (Local Process)

bum spawns a local process and communicates over stdin/stdout:
...
>
> - env `GROK_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; the
>   legacy `GROK_*` compatibility name wins if both are set; the Claude-style
>   name is also measured in **bytes**, not tokens)
> - `config.toml` — user-level (`~/.bum/config.toml` or
>   `$BUM_HOME/config.toml`) **or repo-level**
...
bum mcp list
bum mcp list --json          # Machine-readable output

# Add a stdio server. Everything after -- is the server command, so flags
# like -y reach the server instead of being parsed by bum.
bum mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
bum mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
bum mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
bum mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
bum mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
bum mcp remove github

# Diagnose a server's configuration and connectivity
bum mcp doctor               # Check every configured server
bum mcp doctor github        # Check one server
bum mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `bum mcp add` writes to `~/.bum/config.toml` (or `$BUM_HOME/config.toml`) with `--scope user`. Use `--scope project` to write to the compatibility path `.grok/config.toml` in the current project instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `bum mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`bum mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.
...
MCP servers can be configured per-project by placing a `.grok/config.toml` compatibility file in your repository:
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. bum handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers bum's own OAuth client with the provider.

bum walks from the current directory up to the git repo root, loading project-local `.grok/config.toml` compatibility files at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.bum/config.toml` or `$BUM_HOME/config.toml` | All projects | Lowest |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. bum reads most other config sections only from `~/.bum/config.toml` (or `$BUM_HOME/config.toml`).
...
You can enable or disable MCP servers during a session without restarting bum.
...
bum loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native bum config | `~/.bum/config.toml` (or `$BUM_HOME/config.toml`), project-local `.grok/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.bum/config.toml` (or `$BUM_HOME/config.toml`) or use the legacy compatibility environment variable (`GROK_CURSOR_MCPS_ENABLED`, `GROK_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `bum inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, bum handles the credential flow automatically. When an MCP server requests OAuth credentials, bum opens a browser-based authorization flow and stores the resulting tokens for future use.
...
You must authenticate OAuth-based MCP servers before you can use them. bum stores the resulting tokens under `~/.bum/mcp_credentials.json` (or `$BUM_HOME/mcp_credentials.json`). After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). bum expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
For stdio servers, bum captures the process's standard error to `~/.bum/logs/mcp/<server>.stderr.log` (or `$BUM_HOME/logs/mcp/<server>.stderr.log`), truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.bum/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `bum inspect` to see all loaded MCP servers and their sources:

```bash
bum inspect          # Human-readable
bum inspect --json   # Machine-readable
```
