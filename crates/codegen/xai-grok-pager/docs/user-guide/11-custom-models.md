...
bum connects to custom model endpoints for alternative providers, self-hosted models, and overriding built-in settings. This guide explains how to select models, configure endpoints, and integrate third-party providers.

---

## Default Models

bum's built-in catalog includes models from xAI's Grok family, such as `grok-build`, alongside provider-routed models. Default models require no custom endpoint configuration. Authenticate the required provider with `bum login` or an API key, then start a session.
...
Set a persistent default in `~/.bum/config.toml` (or `$BUM_HOME/config.toml`):
...
bum supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
When you omit `api_backend`, bum uses `chat_completions`.

To send provider-specific authentication or version headers -- for example, Anthropic's `x-api-key` -- use the `extra_headers` field described below. bum sends those headers verbatim with every request to the endpoint.

---

## Configuring Custom Models

Add custom model endpoints in `~/.bum/config.toml` (or `$BUM_HOME/config.toml`) under `[model.<name>]` sections:
...
bum resolves the API key in this order:
...
3. Your signed-in session token (from `bum login`), for a model with no `api_key`/`env_key` of its own
4. The `XAI_API_KEY` environment variable (global fallback; bum also accepts the legacy compatibility name `GROK_CODE_XAI_API_KEY`)

### Context Window

The `context_window` value tells bum when to trigger auto-compaction. When you override a known model, bum inherits that model's context window. When you define a new model and omit `context_window`, bum defaults to 200,000 tokens, so set it explicitly to match your provider.
...
When you override a built-in model, bum starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which bum sends verbatim.
...
Point bum at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `GROK_MODELS_BASE_URL` | Yes | Legacy compatibility knob for the inference base URL. bum fetches the model list from `{base_url}/models`. |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer`. bum also accepts the legacy compatibility name `GROK_CODE_XAI_API_KEY`. |
...
When you use `[endpoints]` with partial model overrides, bum inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, bum uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `bum login` -- the API key is enough.
...
If you point web search at a custom model, you also need a `[model.*]` entry so bum can reach it. Server-side ("backend") web search runs only when the model sets `supports_backend_search = true` (and the build enables backend search); it does not depend on `api_backend`:
