...
bum draws all TUI colors from a central theme. You can switch themes while bum is running, follow your operating system's light or dark appearance, and adjust scrollback layout, animations, and block styling through configuration files.

---

## Available Themes

bum includes five built-in themes, plus an `auto` option that follows your system appearance:
...
Run the `/theme` slash command (alias `/t`) to open the theme picker. As you move through the list with the arrow keys, bum previews each theme in real time. Press Enter to apply and save your choice, or press Escape to revert.
...
Set the theme in `~/.bum/config.toml` (or `$BUM_HOME/config.toml`):
...
Set `theme = "auto"` to have bum follow your operating system's light/dark appearance and switch themes automatically:
...
Once running, bum polls for appearance changes every 5 seconds. Toggling your OS between light and dark mode takes effect within seconds without restarting.
...
On startup, bum detects your terminal's color capability level:
...
When you set `NO_COLOR`, bum emits no color and renders in monochrome.
...
Every theme is defined using full RGB values. At startup, bum quantizes all colors to match the detected capability level. This means:
...
bum sets your terminal cursor to the current theme's `accent_user` color using the OSC 12 escape sequence, to indicate an active bum session. The cursor color is:
...
The setting is persisted in `~/.bum/config.toml` (or `$BUM_HOME/config.toml`) under `[ui].compact_mode` and survives restarts.
