...
bum reads configuration from local config files, environment variables, and
...
3. **config.toml** (`~/.bum/config.toml` or `$BUM_HOME/config.toml`)
...
Location: `~/.bum/config.toml` (or `$BUM_HOME/config.toml`)

If the file does not exist, bum uses built-in defaults. Specify only the values you want to override.

### General Settings

```toml
[cli]
auto_update = false                    # locked off; stock x.ai updates are disabled
...
feedback = false                       # locked off; stock feedback upload is disabled
...
**Disable vim input mode**); bum writes your choice to `[ui] simple_mode` in
`config.toml`.
...
(`/settings` → **Vim scrollback navigation**). bum writes the change to
`[ui] vim_mode` in `~/.bum/config.toml` immediately and applies it to every
...
preference**: whichever mode you last chose explicitly is what a plain `bum`
...
You normally never edit this key by hand — bum writes it whenever you pass an
explicit `--minimal` / `--fullscreen` flag or run `/minimal` / `/fullscreen`.
A plain `bum` launch only reads it. A CLI flag always wins over the config
...
MCP servers can also be configured per-project in the compatibility directory `.grok/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.bum/config.toml` (or `$BUM_HOME/config.toml`).

Priority for `[mcp_servers]` and `[plugins]`: `.grok/config.toml` (current dir) > `<repo-root>/.grok/config.toml` > `~/.bum/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).
...
`bum inspect` reports cells that still need session-start resolution as
...
The `[hints]` table holds small persisted UI preferences — mostly "stop asking me" opt-outs. bum writes these for you when you pick a "don't ask again" / "reset in config.toml" option in the TUI, but you can edit or remove them by hand. Deleting a key restores the default behavior.

`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.bum/config.toml` (or `$BUM_HOME/config.toml`).
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, and the legacy internal item name **grok** (retained for configuration compatibility). |
...
command = "terminal-notifier -title 'bum' -message '$GROK_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
fires, try `condition = "always"` as a fallback. bum supports focus tracking
...
bum's product telemetry is disabled by default and the stock x.ai telemetry
path remains off. Set these fields only when deliberately sending data to your
own infrastructure; collector credentials are operator-supplied.
...
Location: `~/.bum/pager.toml` (or `$BUM_HOME/pager.toml`)
...
Key environment variables. See the README for the complete list. Most
`GROK_*` names below are preserved internal compatibility interfaces; they do
not identify the product or its global home.
...
| Variable | Description |
|----------|-------------|
| `BUM_HOME` | Override the bum product directory (default: `~/.bum`) |
...
| Path | Description |
|------|-------------|
| `~/.bum/config.toml` | Main configuration file |
| `~/.bum/pager.toml` | TUI appearance configuration |
| `~/.bum/auth.json` | Authentication credentials (auto-managed) |
| `~/.bum/sessions/` | Persisted sessions (organized by working directory) |
| `~/.bum/memory/` | Cross-session memory files and index |
| `~/.bum/skills/` | User-scoped skill definitions |
| `~/.bum/plugins/` | User-scoped plugins |
| `~/.bum/agents/` | User-scoped agent definitions |
| `~/.bum/lsp.json` | LSP server configuration (user-scoped) |
| `~/.bum/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
...
|------|--------------------|
| `.grok/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.bum/config.toml`; `.grok` is retained for project compatibility) |
...
|--------|----------|-------|
| User | `~/.bum/lsp.json` | All projects |
