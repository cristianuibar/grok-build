...
bum supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
Plugin hooks automatically receive the internal compatibility environment variables `GROK_PLUGIN_ROOT` and `GROK_PLUGIN_DATA` (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.bum/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `bum plugin marketplace add <source>`)
...
Sources land in `~/.bum/config.toml`:
...
Or in `~/.bum/settings.json` / `~/.claude/settings.json`:
