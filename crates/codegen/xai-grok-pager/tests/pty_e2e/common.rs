...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let grok_home = content.home().join(".bum");
...
/// Seed `~/.bum/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{BUM_HOME|HOME}/.bum/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let grok_home = content.home().join(".bum");
    std::fs::create_dir_all(&grok_home).expect("create .bum");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let grok_home = content.home().join(".bum");
    std::fs::create_dir_all(&grok_home).expect("create .bum");
    // Minimal opt-in only — matches load_config's `{BUM_HOME|HOME}/.bum/config.toml`.
...
/// `BUM_HOME`, returning the exit code (`None` if it never exited within
...
pub(crate) fn run_wrap(wrap_args: &[&str], extra_env: &[(&str, &str)]) -> (Option<u32>, String) {
    ...
    let mut env: Vec<(&str, &str)> = vec![("BUM_HOME", &home_str), ("NO_COLOR", "1")];
