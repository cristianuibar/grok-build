...
//! developer's `~/.bum/auth.json` so the pager skips device-login when
...
/// Prefer explicit path, else the user's real `~/.bum/auth.json`.
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    if let Some(src) = auth_json_source() {
        ...
        eprintln!("pty_auto_mode: no ~/.bum/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("BUM_HOME".into(), grok.display().to_string()),
